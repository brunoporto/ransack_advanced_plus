module RansackAdvancedPlus
  module SavedSearchUtils
    ...
    def get_saved_searches(context)
      @saved_searches = RansackSavedSearch.where(context: context)
    end

    # Return params of Saved Search or search form params
    def get_params_to_search(context)
      if params[:saved_search].present?
        @saved_search = RansackSavedSearch.find_by(id: params[:saved_search], context: context)
      end
    ...
    def update_saved_search(params)
      saved_search = RansackSavedSearch.find_by(id: params[:saved_search], context: params[:context])
